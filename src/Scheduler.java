...
public class Scheduler {
	...
	public void solve() {
		...
		while (!unassigned.isEmpty()) {
			...
			if(redundantsolution != 0)
				System.out.println("Redundant Solution.");
			else
				printSchedule();
	...
	public int eval(Class c, Timeslot t) {
		...
		if (t.room.capacity < c.students.size())
			score -= 100;
		if (Math.abs(c.students.size() - t.room.capacity) > 38)
			score -= Math.abs(c.students.size() - t.room.capacity);
		/*
		 * else score += Math.abs((c.students.size() - t.room.capacity));
		 */
		...
		for (Teacher teacher : c.teachers) {
			if (teacher.preferedDays.isEmpty())
				continue;
			if (teacher.preferedDays.get(0) == null)
				continue;

			int dayIndex = teacher.preferedDays.indexOf(t.day);
			if (dayIndex == -1) {
				score -= 10000;
				c.hadteacherconflict++;
			} else
				score += (5 - dayIndex);
		}
	...
	private void printSchedule() {
		String content = "";
		String times[] = { "08:30", "09:20", "10:20", "11:10", "12:20",
				"13:10", "14:00", "14:55" };
